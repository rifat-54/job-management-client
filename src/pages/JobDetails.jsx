...
const JobDetails = () => {
  ...
  const handleBids = async (e) => {
    ...
    // deadline validation
    if (compareAsc(new Date(), deadline) === 1) {
      return toast.error("deadline crossed!");
    }

    // validation offer deadline validation
    if (compareAsc(new Date(startDate), deadline) === 1) {
      return toast.error("please offer date within the deadline");
    }
    // price validation
    ...
    if (price > parseInt(max_price))
      return toast.error("offer less or at least Maxium price");
